...
from PyQt5.QtWidgets import (
    ...
    QPushButton, QHBoxLayout, QHeaderView, QSpinBox, QLineEdit, QLabel
)
...
class ShoppingCart(QWidget):
    def __init__(self):
        ...
        self.setWindowIcon(QIcon("EmptyIcon.png"))
    ...
    def updateTable(self):
        ...
        for row, item in enumerate(self.items):
            ...
            qtyBox = QSpinBox()
            qtyBox.setValue(self.quantities[row])
            qtyBox.setMinimum(1)
            qtyBox.valueChanged.connect(lambda val, r=row: self.updateQuantity(r, val))
            self.table.setCellWidget(row, 1, qtyBox)
